...
if(@count($res) == 0){
	...
	$query2 = $pdo->query("SELECT * FROM dias where funcionario = '$funcionario' and (data = '$data')");
	$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
	if(@count($res2) > 0){
		$inicio = $res2[0]['inicio'];
		$final = $res2[0]['final'];
		$inicio_almoco = $res2[0]['inicio_almoco'];
		$final_almoco = $res2[0]['final_almoco'];
	}
}
