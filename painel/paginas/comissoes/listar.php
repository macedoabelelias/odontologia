...
if($funcionario2 != ""){
	$query = $pdo->query("SELECT * FROM $tabela where data_lanc >= '$dataInicial' and data_lanc <= '$dataFinal' and pago LIKE '$status' and funcionario = '$funcionario2' and referencia = 'Comissão' ORDER BY pago asc, vencimento asc");
}else{
...
<td>
		...
		<big><a class="btn btn-primary-light btn-sm" href="#" onclick="mostrar('{$descricao}', '{$valorF}', '{$data_lancF}', '{$data_vencF}',  '{$data_pgtoF}', '{$nome_usuario_lanc}', '{$nome_usuario_pgto}', '{$nome_func}', '{$tel_func}', '{$chave_pix_func}', '{$tipo_chave}')" title="Ver Dados"><i class="fa fa-info-circle "></i></a></big>
		
<a href="#" class="btn btn-danger-light btn-sm" onclick="excluir('{$id}')" title="Excluir"><i class="fa fa-trash-can "></i></a>

	<big><a class="btn btn-success-light btn-sm {$visivel}" href="#" onclick="baixar('{$id}', '{$valor}', '{$descricao}', '{$saida}')" title="Baixar Conta"><i class="fa fa-check-square "></i></a></big>
...
<script type="text/javascript">
	function mostrar(descricao, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_pgto, func, tel, pix, tipo_chave){
